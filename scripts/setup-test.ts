...
/**
 * Drive Test Token Setup
 *
 * Generates OAuth token for Drive server tests in local .tokens/ directory.
 *
 * Usage:
 *   npm run test:setup
 */
...
async function setupTest(): Promise<void> {
  ...
  console.log('🔐 Drive Test Token Setup');
  console.log('');
  ...
  console.log(`   Token key: ${email}:drive:token`);
  console.log('');
  console.log('Run `npm run test:unit` to verify Drive API integration');
}
