...
export function assertArraysShape<T extends { type?: string; shape?: string }>(branch: T | undefined, context: string): asserts branch is T & { type: 'success'; shape: 'arrays' } {
  assertSuccess(branch, context);
  assert.equal(branch.shape, 'arrays', `${context}: expected arrays shape, got ${branch.shape}`);
}
